...
public class AudioManager : SingletonMonoBehaviour<AudioManager>
{
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < audioSourceArray.Length; i++)
        {
            ...
            if (i == 0)
            {
                ...
                _bgmSource.volume = PlayerPrefs.GetFloat(BGM_VOLUME_KEY, 0);
            }
            else
            {
    ...
    public void PlaySE(string seName, GameObject gameObject, float vol = SE_VOLUME_DEFULT, float delay = 0.0f, float maxDistance = 100, bool loop = false, bool doChild = true)
    {
        if (!_seDic.ContainsKey(seName))
        {
            Debug.Log(seName + "という名前のSEがありません");
        ...
        //3Dの距離減衰をCustomに
        se3d.rolloffMode = AudioRolloffMode.Linear;
        //再生の時間を遅らせる
    ...
    //Voice
    //=================================================================================
    /// <summary> 3DVoiceを再生します。 </summary>
    /// <param name="voiceName"> 再生するVoice </param>
    /// <param name="gameObject"> 再生したい場所のオブジェクト（this.gameObject） </param>
    /// <param name="vol"> ボリューム </param>
    /// <param name="delay"> 指定した時間だけ再生までの間隔を空ける </param>
    /// <param name="maxDistance"> 音の聞こえる最大距離 </param>
    /// <param name="loop"> ループ再生したいか </param>
    /// <param name="doChild"> 生成場所を子にするかしないか </param>
    public void PlayVoice(string voiceName, GameObject gameObject, float vol = SE_VOLUME_DEFULT, float delay = 0.0f, float maxDistance = 100, bool loop = false, bool doChild = true)
    {
        ...
        if (loop)
        {
            //同じ名前のVoiceは再生しない(重複制御)
            foreach (Transform child in gameObject.transform)
            {
                AudioSource childAudio = child.GetComponent<AudioSource>();
                if (childAudio && childAudio.clip.name == voiceName)
                {
                    return;
                }
            }
        ...
        //オブジェクトの生成
        GameObject voice_E = new GameObject(voiceName);

        //再生する場所のオブジェクトの子にする
        if (doChild)
        {
            voice_E.transform.parent = gameObject.transform;
            voice_E.transform.position = gameObject.transform.position;
        }
        else
        {
            voice_E.transform.position = gameObject.transform.position;
        ...
        //生成したオブジェクトにAudioSourceをつける
        voice_E.AddComponent<AudioSource>();
        voice = voice_E.GetComponent<AudioSource>();
        //クリップにseNameの音をつける
        voice.clip = _voiceDic[_nextVoiceName] as AudioClip;
        //SE用のAudioMixierをつける
        voice.outputAudioMixerGroup = audioMixier.FindMatchingGroups("Voice")[0];
        //音量の調整
        voice.volume = vol;
        //ループ再生するかしないか
        voice.loop = loop;
        ...
        if (loop == false)
        {
            Destroy(voice_E, voice.clip.length + delay);
        }
    ...
    public void PlayBGM(string bgmName, float vol = 0.25f, float fadeSpeedRate = BGM_FADE_SPEED_RATE_HIGH)
    {
        ...
        if (!_bgmSource.isPlaying)
        {
            ...
            PlayerPrefs.SetFloat(BGM_VOLUME_KEY, _bgmVol);
            _stopBGMCheck = false;
        }
    ...
    private void Update()
    {
        ...
        {
            _bgmSource.Stop();
            _bgmSource.volume = PlayerPrefs.GetFloat(BGM_VOLUME_KEY, _bgmVol);
            _isFadeOut = false;
            _stopBGMCheck = true;
