Shader "Unlit/SimpleIBLShaderTextureBump"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // アルベドカラー
                float3 albedoColor = tex2D(_AlbedoMap, i.uv);
                // メタリック(r) スムースネス(a)
                float4 metallicSmoothness = tex2D(_MetallicSmoothnessMap, i.uv);
                // アンビエントオクルージョン
                ...
                float3x3 TBN = float3x3(
                    normalize(i.tangent), normalize(i.binormal), normalize(i.normal));

                // 各種ベクトルを求める
                float3 N = mul(UnpackNormal(tex2D(_NormalMap, i.uv)), TBN); // 法線ベクトル
